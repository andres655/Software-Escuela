        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("¿Seguro que quieres guardar este estudiante?", "Salir", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ...
                    String cmd = String.Format("EXEC Actualizar_Estudiante '{0}', '{1}', '{2}','{3}','{4}','{5}','{6}'", txtNombre_Estudiante.Text.Trim(), txtNumero.Text.Trim(), txtDireccion.Text.Trim(), Convert.ToInt64(TxtCategoria.Text), TxtCedula.Text.Trim(), txtCorreo.Text.Trim(), DateTime.Now);
